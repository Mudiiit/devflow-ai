...
export class NotificationsService {
  ...
  async getInbox(userId: string, limit = 50): Promise<InboxResult> {
    ...
    const unreadCount = await this.notificationsRepository.countUnreadByUserId(userId);
  ...
  async getUnreadCount(userId: string): Promise<{ readonly unreadCount: number }> {
    return {
      unreadCount: await this.notificationsRepository.countUnreadByUserId(userId),
    };
  }

  async markAsRead(userId: string, notificationId: string): Promise<NotificationSummary | null> {
  ...
  streamInbox(userId: string, lastEventId?: string): Observable<MessageEvent> {
    return new Observable<MessageEvent>((subscriber) => {
      ...
      const initialize = async (): Promise<void> => {
        try {
          if (lastEventId) {
            const lastEvent = await this.notificationsRepository.findById(lastEventId);
            cursor = lastEvent?.createdAt ?? null;

            const missed = await this.notificationsRepository.findManyByUserId(userId, {
            ...
            const initial = await this.notificationsRepository.findManyByUserId(userId, {
              limit: 25,
              sort: 'desc',
            });
          ...
          pollTimer = setInterval(() => {
            void this.pollNotifications(userId, cursor, subscriber)
              ...
              .catch((error: unknown) => {
                if (!closed) {
                  subscriber.error(error instanceof Error ? error : new Error('Notification stream failed'));
                }
              });
          }, 5_000);
        } catch (error: unknown) {
