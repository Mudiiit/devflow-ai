...
import { hasPermission, type AppPermission, type AppRole } from '../rbac.types.js';
...
export class OrganizationPermissionGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const request = context.switchToHttp().getRequest<Request & { orgContext?: OrganizationContext }>();
    ...
    return requiredPermissions.every((permission) => hasPermission(membership.role!, permission));
  }
}
