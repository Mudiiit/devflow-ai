...
export class SecretsService {
  constructor(
    private readonly encryptedSecretsRepository: EncryptedSecretsRepository,
  ) {}
  ...
  async getSecret(input: {
    ...
    const scoped = await this.encryptedSecretsRepository.findByScopedKey(
    ...
    const fallback =
      scoped ??
      (input.repositoryId
        ? await this.encryptedSecretsRepository.findByScopedKey(
            input.organizationId,
            null,
            input.key,
          )
